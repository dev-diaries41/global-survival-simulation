...
export class SurvivalSimulation {
    ...
    public async run(): Promise<GlobalState> {
        ...
        for (let year = 1; year <= this.maxYears; year++) {
            ...
            this.globalState.totalResources.food = Math.max(this.globalState.totalResources.food - resourceDepletionRate.food, 0);
            this.globalState.totalResources.energy = Math.max(this.globalState.totalResources.energy - resourceDepletionRate.energy, 0);
            this.globalState.totalResources.water = Math.max(this.globalState.totalResources.water - resourceDepletionRate.water, 0);
